...
public class ImprovedColorPanel extends JPanel implements PreviewContentPanel {
	...
	public ImprovedColorPanel(PreviewPanel pnl) {
		...
		btnPnl = new ButtonPanel(this);
		add(btnPnl);
		btnPnl.setBounds(32, 280, 235, 370);

		slidPnl = new SliderPanel(this);
		add(slidPnl);
	...
	private void updatePanels() {
		if (!isUpdated) {
			...
			Thread selectionDelayWaiterThread = new Thread(() -> {
				try {
			...
			selectionDelayWaiterThread.start();
		}
...
class ButtonPanel extends JPanel {
	...
	private int startSelBtnsRow = 4, colorFuncIncr = 829069;
	private ImprovedColorPanel colPnl;
	private java.util.Queue<ColorButton> selectionBtns;

	public ButtonPanel(ImprovedColorPanel colPnl) {
		super();
		...
		for (int i = 0; i < rowNum; i++) {
			for (int x = 0; x < colNum; x++) {
				if (i < startSelBtnsRow) {
					int c = (x + i * colNum) * colorFuncIncr;
...
class HexPanel extends JPanel {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (int i = 0; i < hexes.length; i++) {
					if (hexes[i].contains(p)) {
						Color[] availableCols = varyColorArray(mainColor);
						colPnl.setColor(availableCols[i]);
						break;
					}
				}
			}
	...
	// Rotates point (x, y) theta degrees around point (a, b)
...
class SliderPanel extends JPanel {
	...
	public SliderPanel(ImprovedColorPanel colPnl) {
		super();
		setLayout(null);
