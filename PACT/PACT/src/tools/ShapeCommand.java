...
public class ShapeCommand implements Command {
	...
	public void doAction(Grid grid, HashMap<String, Integer> mouseInfo) {
		...
		BiConsumer<Graphics2D, int[]> selectedShape = shapeFunctions.get(comboBox.getSelectedIndex());
		selectedShape.accept(gImg, new int[] { startX, startY, Math.abs(x - x2), Math.abs(y - y2) });
		gImg.dispose();
